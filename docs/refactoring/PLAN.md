...
#### 12F. Profile scheduler lifecycle

Результат:

- one-shot и continuous profile loops вынесены в
  `facebook/orchestration/scheduling/scheduler.py`; legacy `_run_once` и
  `_run_continuously` остались composition wrappers;
- `SchedulerConfig` содержит capacity, default/retry rest, discovery и
  max-cycle limits; `SchedulerHooks` инвертирует discovery, profile loading,
  cycle execution, restart-rest calculation и logging;
- scheduler зависит только от узкого `ProfileScheduleState` port;
  concrete JSON store, argparse, global stop event и wall/monotonic clocks подаются
  снаружи;
- independent profile completion, due ordering, max parallel, automatic discovery,
  resume rest, infrastructure retry, no-profile waiting и return codes `0/1/130`
  сохранены;
- существующие integration tests подтвердили max parallel 5,
  queueing для 7 profiles, independent fast/slow cycles и recovery starvation
  protection;
- добавлено 5 focused scheduler tests: empty/error/stop one-shot, resume
  delay, failed-cycle retry schedule и automatic start нового profile без restart;
- legacy orchestrator сократился с 2157 до 2083 строк; scheduler и
  models составляют 174 и 28 строк;
- focused orchestration regression: 115 tests; branch-aware coverage 96%,
  scheduler coverage 98%; полный regression: 637 tests;
- Ruff, strict mypy, architecture/contracts, stage-scoped pre-commit, wheel
  build, frontend production build и gitleaks проходят;
- frontend source/lock не менялись; `npm audit` сохраняет известные 6
  findings (3 moderate, 3 high).
...
#### 12I. Profile evaluation service

Результат:

- загрузка history, baseline, calibration timestamps и recovery context,
  а также вычисление calibration decision вынесены в
  `facebook/orchestration/lifecycle/evaluation.py`;
- `ProfileEvaluationService` зависит от узкого `ProfileStateReader` port и
  pure evaluation policies; concrete JSON store, argparse, subprocess и filesystem
  в lifecycle service не проникают;
- typed `ProfileEvaluation` возвращает decision и весь recovery context,
  нужный profile cycle, без повторных state reads в coordinator;
- regular profile cycle сохраняет exact state-read order, cooldown override,
  quality guard и recovery burst semantics; standalone `evaluate` CLI не загружает
  recovery context и исключает current run из history/baseline как и раньше;
- legacy `_profile_evaluation_policy` import name сохранен exact alias;
  command order, persisted JSON shape, logs и `health.json` не менялись;
- добавлено 3 focused tests: recovery/quality evaluation, CLI current-run
  exclusion и existing-baseline precedence; evaluation module coverage 100% по
  строкам и веткам;
- focused orchestration regression: 135 tests, branch-aware coverage 96%; полный
  regression: 657 tests;
- legacy orchestrator сократился с 1938 до 1901 строки; evaluation module
  составляет 125 строк;
- Ruff, strict mypy, architecture/contracts, stage-scoped pre-commit, wheel
  build, frontend production build и gitleaks проходят;
- frontend source/lock не менялись; `npm audit` сохраняет известные 6
  findings (3 moderate, 3 high).
...
#### 14G3. Orchestrator run и scheduler composition

Результат:

- top-level `run`, сборка `ProfileScheduler`, преобразование CLI scheduling
  options и построение `CalibrationPolicy` вынесены в owning
  `facebook.orchestration.commands`;
- новый run composition получает state store, discovery, profile cycle, clock,
  sleep, stop condition и logging через immutable `RunCommandHooks`, не
  импортируя legacy service;
- one-shot/continuous semantics, fail-fast initial discovery, non-fail-fast
  periodic discovery, independent profile scheduling и schedule logging
  сохранены без изменения policy или defaults;
- legacy `_run` остался composition adapter; прежние `_calibration_policy`,
  `_profile_rest_seconds`, `_profile_schedule_policy`,
  `_remaining_profile_rest_seconds` и `RecoverySchedulePolicy` сохранены как
  compatibility aliases, а runtime monkeypatch seams продолжают связываться
  при каждом вызове;
- legacy orchestrator сокращен с 1 090 до 934 строк; canonical `models`,
  `policy` и `run` имеют 39, 72 и 136 строк;
- добавлено 2 focused contracts для alias identity и one-shot composition;
  focused orchestration/CLI/architecture regression: 225 тестов, полный
  regression: 868 тестов;
- wheel содержит новые canonical modules; Ruff, strict mypy, stage-scoped
  pre-commit, frontend production build и gitleaks проходят;
- frontend source/lock не менялись; `npm audit` сохраняет известные 6 findings
  (3 moderate, 3 high).

Этап 14G остается `IN_PROGRESS`: profile cycle, collection pipeline,
...
#### 14I2. Enrichment и calibration CLI cutover

Результат:

- manual calibration и relevant-only enrichment release paths переведены на
  `facebook-spy calibrate` и `facebook-spy enrich`; README больше не направляет
  оператора к service wrappers;
- CLI contract suite фиксирует оба unified gateway command, их прежние options
  и help surface; для enrichment добавлен отсутствовавший release-path contract;
- удалены агрегирующие executable wrappers `facebook_ad_enricher.py` и
  `facebook_calibrator.py`; architecture denylist запрещает возврат файлов и
  production imports;
- interest-safety tests используют `EnrichmentService`, post policy и
  Playwright recovery adapter напрямую; blocked rows по-прежнему не получают
  browser access;
- calibration tests используют accounting functions, `cli.legacy` adapter,
  parser и target-engagement owner напрямую; wrapper-level aliases и
  Playwright error re-export удалены без потери behavioral assertions;
- focused cutover regression: 212 тестов, полный regression: 898 тестов;
- wheel содержит canonical enrichment/calibration commands и services, но не
  содержит два удалённых wrappers; Ruff, mypy, architecture/contracts,
  stage-scoped pre-commit, frontend production build и gitleaks проходят;
- frontend source/lock не менялись; `npm audit` сохраняет известные 6 findings
  (3 moderate, 3 high).

Этап 14I остается `IN_PROGRESS`: collection runner и orchestrator имеют
