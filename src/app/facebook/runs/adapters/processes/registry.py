...
class PendingImport(Protocol):
    @property
    def pending_count(self) -> int: ...


class FacebookRunnerRegistry:
    def __init__(self, config: Config, importer: FacebookAdsImporter) -> None:
        self.config = config
    ...
    async def start(self, run: Run | FacebookRun) -> None:
    ...
    def _command(self, run: Run | FacebookRun, run_dir: Path) -> list[str]:
        octo_profile_uuid = (
    ...
    async def _monitor(
        ...
        stream: FacebookAdsStreamingImportSession | None = None
    ...
    async def _poll_stream_import(
        self,
        run_id: UUID,
        stream: FacebookAdsStreamingImportSession,
    ) -> None:
        try:
            async with SessionFactory() as session:
                async with UnitOfWork(session) as uow:
    ...
    async def _drain_stream_import(
        self,
        run_id: UUID,
        stream: FacebookAdsStreamingImportSession,
    ) -> None:
        deadline = asyncio.get_running_loop().time() + self._stream_drain_timeout(
            stream
        )
    ...
    def _stream_drain_timeout(self, stream: PendingImport) -> float:
