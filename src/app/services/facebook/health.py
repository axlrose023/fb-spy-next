...
def collect_run_metrics(
    ...
    *,
    ...
    ads = _load_ads(run_path)
    meta = _load_json(run_path / "run_meta.json", default={})
    summary = _load_json(run_path / "summary.json", default={})

    started_at = _clean(meta.get("started_at") or summary.get("started_at"))
    finished_at = _clean(summary.get("finished_at")) or _last_captured_at(ads)
    elapsed_seconds = _float_or_none(summary.get("elapsed_seconds"))
    if elapsed_seconds is None:
        elapsed_seconds = _elapsed_from_timestamps(started_at, finished_at)
    if elapsed_seconds is None:
        elapsed_seconds = default_elapsed_seconds

    requested_minutes = _float_or_none(
    ...
    scrolls = _int_or_none(summary.get("scrolls"))
    if scrolls is None:
        scrolls = default_scrolls
    refreshes = _int_or_none(summary.get("refreshes"))
    captured_candidates = _int_or_none(summary.get("captured_candidates"))
    duplicate_fb_ad_ids = int(_int_or_none(summary.get("duplicate_fb_ad_ids")) or 0)
    stop_reason = _clean(summary.get("stop_reason"))

    profile_country = _clean(meta.get("profile_country"))
    country_target = expected_country or profile_country
    by_type = _count_by(ads, "ad_type")
    resolved_ads = [
        ad for ad in ads if _clean(ad.get("landing_full") or ad.get("landing_clean"))
    ]
    domains = [_domain_key(_domain_value(ad)) for ad in ads]
    domains = [domain for domain in domains if domain]
    domain_counts = _counts(domains)
    screenshot_attempted = sum(1 for ad in ads if _clean(ad.get("screenshot")))
    screenshot_ok = sum(
        1
        for ad in ads
        if _clean(ad.get("screenshot")) and ad.get("screenshot_ok") is not False
    )
    countries = [_clean(ad.get("country")) for ad in ads if _clean(ad.get("country"))]
    country_match_rate = None
    if countries and country_target:
        normalized_target = _normalize(country_target)
        country_match_rate = sum(
            1 for country in countries if _normalize(country) == normalized_target
        ) / len(countries)

    relevance_classified_ads = sum(1 for ad in ads if _has_relevance(ad))
    relevance_coverage = _safe_div(relevance_classified_ads, len(ads))
    relevance_known = (
    ...
    relevant_rate = None
    if relevance_known:
        relevant_ads = sum(1 for ad in ads if _is_relevant(ad))
        relevant_rate = _safe_div(relevant_ads, relevance_classified_ads)
    ...
    hours = _safe_div(elapsed_seconds, 3600.0)
    profile_uuid = _clean(meta.get("octo_profile_uuid"))
    profile_expected = expected_country or _clean(meta.get("expected_country"))
    geo_match = _geo_match(profile_expected, profile_country)

    return RunMetrics(
        run_dir=str(run_path),
        collector_metric_version=(
            _int_or_none(meta.get("collector_metric_version")) or 1
        ),
        ...
        octo_headless=(
            _bool_or_none(meta.get("octo_headless"))
        ...
        unique_landing_clean=len(
            {
                _clean_landing_key(ad.get("landing_clean") or ad.get("landing_full"))
                for ad in resolved_ads
                if _clean_landing_key(ad.get("landing_clean") or ad.get("landing_full"))
            }
        ),
        unique_fb_ad_ids=len(
            {_clean(ad.get("fb_ad_id")) for ad in ads if _clean(ad.get("fb_ad_id"))}
        ),
        unique_advertisers=len(
            {
                _normalize(_clean(ad.get("advertiser")))
                for ad in ads
                if _clean(ad.get("advertiser"))
            }
        ),
        ...
        screenshot_ok_rate=_safe_div(screenshot_ok, screenshot_attempted),
        ...
        ads_per_hour=_rate(len(ads), hours),
        target_per_hour=_rate(target_ads, hours),
        resolved_per_hour=_rate(len(resolved_ads), hours),
        ads_per_100_scrolls=_per_100(len(ads), scrolls),
        target_per_100_scrolls=_per_100(target_ads, scrolls),
        resolved_per_100_scrolls=_per_100(len(resolved_ads), scrolls),
...
def _load_ads(run_dir: Path) -> list[dict[str, Any]]:
...
def _last_captured_at(ads: list[dict[str, Any]]) -> str | None:
    values = [
        _clean(ad.get("captured_at")) for ad in ads if _clean(ad.get("captured_at"))
    ]
    return max(values) if values else None


def _elapsed_from_timestamps(
...
def _geo_match(expected: str | None, observed: str | None) -> bool:
    if not expected or not observed:
        return True
    return _normalize(expected) == _normalize(observed)


def _count_by(ads: list[dict[str, Any]], key: str) -> dict[str, int]:
    counts: dict[str, int] = {}
    for ad in ads:
        value = _clean(ad.get(key)) or ""
...
def _domain_value(ad: dict[str, Any]) -> str:
    return (
        _clean(
...
def _clean_landing_key(value: Any) -> str:
    text = _clean(value)
    if not text:
        return ""
    try:
...
def _has_relevance(ad: dict[str, Any]) -> bool:
...
def _is_relevant(ad: dict[str, Any]) -> bool:
...
def _rate(count: int | None, hours: float | None) -> float | None:
    if count is None or not hours:
        return None
    return count / hours


def _per_100(count: int, total: int | None) -> float | None:
    if not total:
        return None
